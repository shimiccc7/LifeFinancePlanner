...
var defaults={income:0,travelGoal:0,travelNow:0,travelMonthly:0,esppPct:8,quests:[false,false,false,false,false],fontScale:1};
var defs={
...
var raw={};try{raw=JSON.parse(localStorage.getItem('lfp')||'{}')}catch(e){}
var s=Object.assign({},defaults,raw);if(!s.fontScale)s.fontScale=1;
...
if(!s.subcategories){s.subcategories={};for(var k in defs)s.subcategories[k]=defs[k].subs.map(function(n){return [n,0]});}
for(var k in defs){
...
function updateStock(){
 ...
 var espp=(Number(s.income)||0)*(Number(s.esppPct)||0)/100;
 var spent=Object.keys(defs).reduce((a,k)=>a+total(k),0)+espp;
 ...
 var r=s.income?Math.max(0,Math.min(100,((s.income-spent)/s.income)*100)):0;document.getElementById('safeBar').style.width=r+'%';document.getElementById('safeTxt').textContent='月收入扣除規劃後保留 '+r.toFixed(1)+'% 現金彈性';
 updateAllocation();
}
function bindBudget(){
 document.querySelectorAll('.budget-toggle').forEach(b=>b.onclick=()=>{var g=b.closest('.budget-group');g.classList.toggle('open');b.textContent=g.classList.contains('open')?'−':'＋'});
 document.querySelectorAll('.subamt').forEach(e=>e.oninput=()=>{var k=e.dataset.k,i=+e.dataset.i;s.subcategories[k][i][1]=Math.max(0,Number(e.value)||0);e.closest('.budget-group').querySelector('.budget-total').textContent=money(total(k));persist();calc()});
 ...
 document.querySelectorAll('.del').forEach(b=>b.onclick=()=>{var k=b.dataset.k;s.subcategories[k].splice(+b.dataset.i,1);persist();renderBudget();(()=>{var g=document.querySelector('[data-group="'+k+'"]');if(g){g.classList.add('open');var bt=g.querySelector('.budget-toggle');if(bt)bt.textContent='−'}})()});
 document.querySelectorAll('.add').forEach(b=>b.onclick=()=>{var k=b.dataset.k;s.subcategories[k].push(['新項目',0]);persist();renderBudget();var g=document.querySelector('[data-group="'+k+'"]');if(g){g.classList.add('open');var bt=g.querySelector('.budget-toggle');if(bt)bt.textContent='−'}});
}
function renderBudget(){
 document.getElementById('budgetRows').innerHTML='<div class="row"><b>可分配月收入</b><input id="incomeInput" type="number" inputmode="numeric" value="'+(s.income||0)+'"></div>'+Object.keys(defs).map(function(k){var d=defs[k];return '<div class="budget-group" data-group="'+k+'"><div class="budget-head"><button type="button" class="budget-toggle">＋</button><div class="budget-title"><b>'+d.name+'</b></div><div class="budget-total">'+money(total(k))+'</div></div><div class="budget-children">'+(s.subcategories[k]||[]).map((x,i)=>'<div class="subrow"><input class="subname" data-k="'+k+'" data-i="'+i+'" value="'+String(x[0]).replace(/"/g,'&quot;')+'"><input class="subamt" data-k="'+k+'" data-i="'+i+'" type="number" inputmode="numeric" min="0" value="'+(Number(x[1])||0)+'"><button type="button" class="del" data-k="'+k+'" data-i="'+i+'">×</button></div>').join('')+'<button type="button" class="add" data-k="'+k+'">＋ 新增子分類</button></div></div>').join('');
...
function render(){
 ...
 var rem=Math.max(0,s.travelGoal-s.travelNow),m=Math.max(1,Math.ceil(rem/Math.max(1,s.travelMonthly)));document.getElementById('travelNeed').textContent='照目前速度約 '+m+' 個月達標';document.getElementById('travelBar').style.width=(s.travelGoal?Math.min(100,s.travelNow/s.travelGoal*100):0)+'%';
 ...
 var xp=s.quests.filter(Boolean).length*20;var xpEl=document.getElementById('xp');if(xpEl)xpEl.textContent=xp;var levelEl=document.getElementById('level');if(levelEl)levelEl.textContent=xp>=80?'Gold':xp>=40?'Silver':'Bronze';
...
boot()
+Math.round(Number(n)||0).toLocaleString('zh-TW');
